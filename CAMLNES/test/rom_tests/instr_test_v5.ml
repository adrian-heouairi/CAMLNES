...
let rec get_message_rec sofar =
  ...
  let char = if char = '\n' then ' ' else char in
  if char = '\000' then sofar else get_message_rec (sofar @ [ char ])

let get_message () = String.of_seq (List.to_seq (get_message_rec []))

exception Break_loop

let run_test name logging =
  if logging then Cpu.enable_logging @@ "cpu-" ^ name ^ ".log"
  else Cpu.disable_logging ();
  ...
  | exc ->
      print_endline
        ("Exception occurred during test " ^ name ^ ": "
