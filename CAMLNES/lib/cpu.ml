...
let state =
  {
    ...
    index_register_Y = 0;
    program_counter = -1;
    (* Set this to what is contained in $FFFC in the cartridge *)
    stack_pointer = 0xFD;
    carry_flag = false;
    zero_flag = false;
...
let enable_logging filename =
  ...
  if filename = "" then logging.log <- stdout
  else logging.log <- open_out_bin filename
...
let _ADC (calculated_addr, byte_at_addr) =
  ...
  state.overflow_flag <-
    lnot (state.accumulator lxor byte_at_addr)
    land (state.accumulator lxor addition)
    land 0x80
    > 0;
...
let _ASL (calculated_addr, byte_at_addr) =
  ...
  if calculated_addr = -1 then state.accumulator <- value2
  else Bus.write calculated_addr value2

let _BCC (calculated_addr, byte_at_addr) =
  if not state.carry_flag then branch byte_at_addr

let _BCS (calculated_addr, byte_at_addr) =
  if state.carry_flag then branch byte_at_addr

let _BEQ (calculated_addr, byte_at_addr) =
...
let _BMI (calculated_addr, byte_at_addr) =
  if state.negative_flag then branch byte_at_addr

let _BNE (calculated_addr, byte_at_addr) =
  if not state.zero_flag then branch byte_at_addr

let _BPL (calculated_addr, byte_at_addr) =
  if not state.negative_flag then branch byte_at_addr

let _BRK (calculated_addr, byte_at_addr) =
  (* Bit 4 to 1 *)
...
let _BVS (calculated_addr, byte_at_addr) =
  if state.overflow_flag then branch byte_at_addr
...
let _DEX (calculated_addr, byte_at_addr) =
  state.index_register_X <-
    (if state.index_register_X = 0 then 255 else state.index_register_X - 1);
...
let _DEY (calculated_addr, byte_at_addr) =
  state.index_register_Y <-
    (if state.index_register_Y = 0 then 255 else state.index_register_Y - 1);
...
let _JSR (calculated_addr, byte_at_addr) =
  let pc =
    if state.program_counter = 0 then 0xFFFF else state.program_counter + 2
  in
  stack_push (pc lsr 8);
  stack_push (pc land 255);
  state.program_counter <- calculated_addr
...
let _LSR (calculated_addr, byte_at_addr) =
  ...
  state.zero_flag <- result = 0;
  if calculated_addr = -1 then state.accumulator <- result
...
let _PHP (calculated_addr, byte_at_addr) =
  (* Bit 4 to 1 *)
  stack_push @@ (status_to_byte () lor 0b0011_0000)
...
let _SBC (calculated_addr, byte_at_addr) = _ADC (-1, byte_at_addr lxor 255)
...
let _STA (calculated_addr, byte_at_addr) =
  Bus.write calculated_addr state.accumulator

let _STX (calculated_addr, byte_at_addr) =
  Bus.write calculated_addr state.index_register_X

let _STY (calculated_addr, byte_at_addr) =
  Bus.write calculated_addr state.index_register_Y
...
let _TXS (calculated_addr, byte_at_addr) =
  state.stack_pointer <- state.index_register_X
...
let _ARR (calculated_addr, byte_at_addr) =
  _AND (calculated_addr, byte_at_addr);
  state.overflow_flag <-
    get_nth_bit 6 state.accumulator <> get_nth_bit 7 state.accumulator;
...
let _LAX (calculated_addr, byte_at_addr) =
  _ORA (0, 0xFF);
  (* 0xEE for some 6502 CPUs, but 0xFF for the NES *)
  _AND (calculated_addr, byte_at_addr);
  _TAX (-1, -1)
...
let run_next_instruction () =
  if state.nmi && not state.nmi_launched then (
    (* TODO: I can't figure out why the program doesn't work when I move the parenthese at the beginning of the line *)
    state.nmi <- false;
  ...
  let () = instruction_function resolved_addr in
  ...
  | _ ->
      state.program_counter <-
        (state.program_counter + instruction_size) mod 65536
